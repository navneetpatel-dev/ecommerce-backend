...
describe('pdfFormatters', () => {
  it('groups rupees with the Indian lakh/crore pattern', () => {
    ...
    assert.equal(formatInrAmount('2364.16'), '2,364.16');
    assert.equal(formatPdfMoney('2364.16'), 'Rs 2,364.16');
  });
  ...
  it('converts amounts to Indian-system words', () => {
    ...
    assert.equal(rupeesInWords('2364.16'), 'Rupees Two Thousand Three Hundred Sixty Four and Sixteen Paise Only');
  });
});
